...
<main>
  ...
  <section class="panel">
    ...
    <div id="campaignPlaceholder" class="campaign-placeholder"><p>Select a prospect</p></div>
...
<section class="products-section">
  <div class="panel-header"><h2>AI Campaign Generator</h2><span class="badge">Segment-Aware</span></div>
  <div class="panel" style="max-width:100%">
    ...
    <div id="aiCampaignPlaceholder" style="color:var(--text-secondary);font-size:0.9rem;padding:1rem 0;">Select a segment and click Generate.</div>
...
function getToken() {
  return fetch(API + "/auth/token")
    ...
    .catch(function(e){ setStatus(false); console.error("Auth failed:", e); });
}
  ...
  fetch(url)
    ...
    .then(function(d){
      ...
      (d.prospects||[]).forEach(function(p,i){
        ...
        var nm = document.createElement("div"); nm.className="prospect-name"; nm.textContent=p.company_name||"Unknown";
        var meta = document.createElement("div"); meta.className="prospect-meta"; meta.textContent=p.industry||p.account_value||"";
        ...
        info.appendChild(nm); info.appendChild(meta); info.appendChild(ct);
...
function selectProspect(p) {
  ...
  fetch(url,{method:"POST"}).then(function(r){return r.json();}).then(function(d){
    ...
    if(p.contact_id&&p.contact_id!=="None") writeHubSpotNote(p.contact_id,p.company_name,d.campaign||".");
...
function generateAICampaign(){
  ...
  output.style.display="none"; placeholder.style.display="none";
  ...
  .catch(function(e){btn.textContent="Generate Campaign";btn.disabled=false;status.textContent="Error: "+e.message;placeholder.style.display="block";});
}
...
function loadShopifyChart(){
  ...
  .then(function(d){
    ...
    var cm={total_orders:"rgba(77,159,204,0.85)",total_revenue:"rgba(0,230,118,0.85)",new_customers:"rgba(221,0,0,0.85)",returning_customers:"rgba(163,98,20,0.85)",vip_customers:"rgba(255,200,0,0.85)"};
...
function loadProducts(){
  ...
  .then(function(r){return r.json();})
  .then(function(data){
    var products=Array.isArray(data)?data.value||data.data||[]);
